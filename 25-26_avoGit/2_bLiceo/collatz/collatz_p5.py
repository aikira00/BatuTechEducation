...
indice = int(input("Quale sequenza vuoi visualizzare? (1 per la prima, 2 per la seconda, ...): "))

# Verifica che l'indice sia valido
if 1 <= indice <= len(list_val_collatz_all):
    # Recupera la sequenza e i passi selezionati
    sequenza_scelta = list_val_collatz_all[indice-1]
    # devo trasofrmarlo in una lista
    list_passi = list(range(len(sequenza_scelta)))
    numero_iniziale = input_list_collatz[indice-1]  # Il numero iniziale corrispondente
