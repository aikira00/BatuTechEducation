public class MatrixADDConcRow {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < A.length; i++) {
            threads[i] = new RowAdder(i, A, B, C);
            threads[i].start();
        }
        ...
        System.out.println("Tempo con un thread per riga: " + (end - start)  + " ms");
    }
...
class RowAdder extends Thread {
    ...
    public RowAdder(int row, int[][] A, int[][] B, int[][] C) {
        this.row = row;
        this.A = A;
        this.B = B;
        this.C = C;
