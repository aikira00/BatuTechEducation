...
int main() {
   ...
   if (ret_val_primo_figlio == 0) {
      ...
      if (ret_val_primo_nipote == 0 ) {
         ...
         sleep(20);
         printf("[NIPOTE1] PID: %d termino \n", getpid());
         exit(0);
      }
      //else { no bisogno
      printf("[PADRE1] PID: %d  ho creato nipote %d \n", getpid(), ret_val_primo_nipote);
      sleep(20);
      printf("[PADRE1] PID: %d termino \n", getpid());
      exit(0);
    //  }
   }
  // else { no bisogno
      ...
      sleep(20);
      printf("[NONNO] PID: %d termino \n", getpid());
      //}
  // }
   printf("Questa print chi la esegue? %d", getpid());
   return 0;
}