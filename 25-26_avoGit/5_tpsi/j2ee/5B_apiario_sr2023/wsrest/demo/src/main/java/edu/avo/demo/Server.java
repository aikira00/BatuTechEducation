...
public class Server {
    ...
    public int insertMiele(Miele miele) {
        try {
            ...
            int num = statement.executeUpdate(); //num righe interessate
            return num;
        } catch (SQLException ex) {
            throw new RuntimeException(ex);
        }
    ...
    public List<Apiario> selectApiari() {
        try {
            List<Apiario> list = new ArrayList<>();
            statement = connection.prepareStatement("Select * from apiari, province where idprovincia=province.id");
            resultSet = statement.executeQuery();
            while (resultSet.next()) {
                list.add(new Apiario(resultSet.getInt(1),resultSet.getString(2), resultSet.getString(3), resultSet.getInt(4), resultSet.getString(5), resultSet.getString(7)));
            }
            return list;
        } catch (SQLException ex) {
            throw new RuntimeException(ex);
    ...
    public List<Apiario> selectApiariByProvincia(String provincia) {
        try {
            ...
            resultSet = statement.executeQuery();
            while (resultSet.next()) {
                // colunm index 7 is nome from province
                list.add(new Apiario(resultSet.getInt(1),resultSet.getString(2), resultSet.getString(3), resultSet.getInt(4), resultSet.getString(5), resultSet.getString(7)));
            }
            return list;
        } catch (SQLException ex) {
    ...
    public List<String> selectProvince() {
        try {
            ...
            statement = connection.prepareStatement("Select * from province");
           // statement.setString(1, provincia);
            resultSet = statement.executeQuery();
            while (resultSet.next()) {
